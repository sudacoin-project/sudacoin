...
#if defined(NDEBUG)
# error "Sudacoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x943fe113063b11c5594f0b7bbb42fb92f4db450af79807cde43993b8e5b53461");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Sudacoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Sudacoin Signed Message:\n";
...
{
    // Sudacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Sudacoin: 420k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Sudacoin: 3 days
static const int64 nTargetSpacing = 2 * 60; // Sudacoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Sudacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Sudacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x2cd7bfb8a8c98924d021c1c08f7277bbadd025b7a7338b2b226c391dd5c13383");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Omdurman  25/Jan/2023 Sudacoin took the control";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04717bf08432cb3a487aec0df0b8fc286bd436290dec854919f722999cafecc5dfaf3777670e15256f4ba3b73b932f59515f1d26a85ce3438a9b4116bb7b8e07a0") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1674675595;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084853865;


        if (fTestNet)
        {
            block.nTime    = 1674675624;
            block.nNonce   = 386334567;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xb066244aa3416b3164bae84ade06ee2a0e77315bccbc8a20a76b182a83f769c7"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc3, 0xb7, 0xdf }; // Sudacoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SudacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SudacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SudacoinMiner(CWallet *pwallet)
{
    printf("SudacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("sudacoin-miner");
    ...
    try { loop {
        ...
        printf("Running SudacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SudacoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SudacoinMiner, pwallet));
}
